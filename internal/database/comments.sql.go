...
func (q *Queries) GetAllCommentsForPost(ctx context.Context, postID pgtype.UUID) ([]GetAllCommentsForPostRow, error) {
	rows, err := q.db.Query(ctx, getAllCommentsForPost, postID)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
	var items []GetAllCommentsForPostRow
	for rows.Next() {
